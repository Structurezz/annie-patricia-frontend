...
function ProductCard({ p }: { p: typeof menProducts[0] }) {
...
export default function MenHome() {
  ...
  const bestsellers = menProducts.filter(p => p.badge === "BESTSELLER").slice(0, 8);
  const newArrivals = menProducts.filter(p => p.badge === "NEW").slice(0, 8);
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* HERO */}
      <section className="relative h-[82vh] min-h-[500px] overflow-hidden bg-[#0B0A09]">
        <img src="https://images.unsplash.com/photo-1507679799987-c73779587ccf?w=1600&q=85" alt="Men" className="absolute inset-0 w-full h-full object-cover opacity-65" />
        ...
        <div className="relative z-10 h-full flex flex-col justify-center px-8 md:px-16 lg:px-24 max-w-xl">
          <motion.p initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .5 }}
            className="text-[10px] tracking-[.5em] text-[#C9A84C] font-inter uppercase mb-4">Men's Collection · SS 2025</motion.p>
          <motion.h1 initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .65, delay: .1 }}
            ...
            The Gentleman's<br /><em className="italic text-[#C9A84C]">Edit.</em>
          </motion.h1>
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .4 }} className="flex gap-3">
            <Link to="/men/shop" className="px-8 py-3.5 bg-[#C9A84C] text-[#0B0A09] text-[11px] font-inter font-semibold tracking-[.2em] hover:bg-[#E2C97E] transition-colors">SHOP ALL MEN</Link>
            <Link to="/category?category=Agbada" className="px-8 py-3.5 border border-white/40 text-white text-[11px] font-inter tracking-[.2em] hover:border-white hover:bg-white/10 transition-colors">AGBADA</Link>
          </motion.div>
        </div>
      </section>

      {/* CATEGORY STRIP */}
      <div className="bg-[#0B0A09] border-b border-white/8">
        <div className="max-w-screen-xl mx-auto px-5 lg:px-10 py-4 flex gap-2 overflow-x-auto scrollbar-hide">
          {menCategories.slice(0, 6).map(cat => (
            <Link key={cat} to={`/category?category=${cat}`}
              className="flex-shrink-0 px-4 py-2 text-[10px] font-inter tracking-[.15em] border border-white/15 text-white/50 hover:border-[#C9A84C] hover:text-[#C9A84C] transition-colors">
              {cat.toUpperCase()}
            </Link>
          ))}
          <Link to="/men/shop" className="flex-shrink-0 px-4 py-2 text-[10px] font-inter tracking-[.15em] bg-[#C9A84C] text-[#0B0A09]">VIEW ALL →</Link>
        </div>
      </div>
      ...
      {/* AGBADA EDITORIAL */}
      <section className="relative overflow-hidden h-[420px] md:h-[500px] mx-5 lg:mx-10 mb-14 md:mb-18 bg-[#0B0A09]">
        <img src="https://images.unsplash.com/photo-1591400073680-d4a22e58efd8?w=1400&q=85" alt="Agbada" className="absolute inset-0 w-full h-full object-cover opacity-55" />
        <div className="absolute inset-0 bg-gradient-to-r from-[#0B0A09]/85 to-transparent" />
        <div className="relative z-10 h-full flex flex-col justify-center px-10 md:px-16 max-w-lg">
          <p className="text-[10px] tracking-[.4em] text-[#C9A84C] font-inter uppercase mb-3">Traditional Wear</p>
          <h2 className="font-cormorant text-[clamp(2.2rem,5vw,4rem)] font-light text-white mb-5">Command the room in hand-crafted Agbada.</h2>
          <Link to="/category?category=Agbada" className="self-start px-8 py-3 border border-[#C9A84C] text-[#C9A84C] text-[10px] font-inter tracking-[.2em] hover:bg-[#C9A84C] hover:text-[#0B0A09] transition-colors">SHOP AGBADA</Link>
        </div>
      </section>
      ...
      <section className="bg-[#F7F4EF] py-14 md:py-18">
        <div className="max-w-screen-xl mx-auto px-5 lg:px-10">
          <div className="flex items-end justify-between mb-8">
            <div>
              <p className="text-[10px] tracking-[.4em] text-[#C9A84C] font-inter uppercase mb-2">Top Picks</p>

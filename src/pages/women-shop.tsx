...
export default function WomenShop() {
  ...
  const cats = ["All", ...womenCategories];

  const products = useMemo(() => {
    let list = cat === "All" ? womenProducts : womenProducts.filter(p => p.category === cat);
  ...
  const handleAdd = (p: typeof womenProducts[0], e: React.MouseEvent) => {
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden">
      ...
      {/* Hero banner */}
      <div className="relative bg-[#F5F0E8] border-b border-[#E0DBD4] overflow-hidden">
        <div className="absolute inset-0">
          <img src="https://images.unsplash.com/photo-1490481651871-ab68de25d43d?w=1400&q=80" alt="" className="w-full h-full object-cover opacity-10" />
        </div>
        <div className="relative max-w-screen-xl mx-auto px-6 lg:px-16 py-16 md:py-20">
          <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:.7}}>
            <div className="flex items-center gap-3 mb-3">
              <Link to="/women" className="text-[9px] tracking-[0.4em] text-[#C9A84C] font-inter uppercase hover:text-[#0A0908] transition-colors">Women</Link>
              <span className="text-[#E0DBD4]">/</span>
              <span className="text-[9px] tracking-[0.4em] text-[#7A7571] font-inter uppercase">Shop</span>
            </div>
            <h1 className="font-cormorant text-[clamp(3rem,7vw,5.5rem)] font-light text-[#0A0908] leading-none mb-3">
              Women's <em className="italic text-[#C9A84C]">Collection</em>
            </h1>
            <p className="text-[#7A7571] font-inter text-sm">{products.length} pieces crafted by Nigeria's finest artisans</p>
          </motion.div>
        </div>
      </div>
          ...
          <div className="flex items-center gap-3 overflow-x-auto scrollbar-hide">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              <select value={sort} onChange={e=>setSort(e.target.value)}
                className="text-[11px] border border-[#E0DBD4] px-3 py-2 text-[#0A0908] font-inter focus:outline-none focus:border-[#0A0908] bg-white cursor-pointer">
                ...
                <motion.article key={p.id} layout initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} exit={{opacity:0}} transition={{delay:(i%4)*0.05,duration:.4}} className="group">
                  <Link to={`/product/${p.id}`}>
                    <div className="relative overflow-hidden bg-[#F0EDE8] aspect-[3/4] mb-3">
                      ...
                      {p.badge && (
                        <span className={`absolute top-3 left-3 text-[9px] font-bold tracking-[0.2em] px-2.5 py-1 ${p.badge==="SALE"?"bg-red-500 text-white":p.badge==="BESTSELLER"?"bg-[#C9A84C] text-[#0A0908]":"bg-[#0A0908] text-white"}`}>
                          {p.badge}
                        </span>
                      )}
                      <div className="absolute inset-x-0 bottom-0 flex translate-y-full group-hover:translate-y-0 transition-transform duration-300 z-10">
                        <button onClick={e=>handleAdd(p,e)} className={`flex-1 py-3 text-[10px] font-semibold tracking-[0.2em] transition-colors ${addedId===p.id?"bg-[#C9A84C] text-[#0A0908]":"bg-white text-[#0A0908] hover:bg-[#0A0908] hover:text-white"}`}>
                          {addedId===p.id?"✓ ADDED":"ADD TO BAG"}
                        </button>
                        <button onClick={e=>toggleWish(p.id,e)} className={`px-4 bg-white border-l border-[#E0DBD4] text-base hover:bg-[#F0EDE8] transition-colors ${wish.has(p.id)?"text-rose-500":"text-[#0A0908]"}`}>
                          {wish.has(p.id)?"♥":"♡"}
                        </button>
                      </div>
                    </div>

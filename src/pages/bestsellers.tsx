...
export default function Bestsellers() {
  ...
  // Top-3 podium — always most popular overall bestsellers
  const { products: podium, loading: loadingPodium } = useProducts({
    badge: "BESTSELLER", limit: 3, sort: "soldCount", order: "desc",
  });
  ...
  return (
    <div className="min-h-screen bg-[#FAF8F5] font-inter text-[#0B0A09]">
      <Topbar />

      {/* ── Hero ── */}
      <div className="relative bg-[#F5F0E8] overflow-hidden">
        <img
          src="https://images.unsplash.com/photo-1441986300917-6467269125f2?w=1400&q=80"
          alt="" className="absolute inset-0 w-full h-full object-cover opacity-[0.08]"
        />
        <div className="relative max-w-screen-xl mx-auto px-6 lg:px-12 py-20 md:py-28">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <p className="text-[#7A7571] text-sm">
              {loading ? "—" : total.toLocaleString()} pieces loved across Nigeria and beyond
            </p>
          </motion.div>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}
            className="flex flex-wrap gap-6 mt-8">
            {["★ 4.9/5 Average Rating", "🚚 Free Delivery Over ₦50k", "↩️ 30-Day Returns"].map(t => (
              <span key={t} className="text-[10px] text-[#7A7571]">{t}</span>
            ))}
          </motion.div>
        </div>
      </div>
      ...
      {/* ── Top-3 podium ── */}
      <div className="max-w-screen-xl mx-auto px-5 lg:px-12 py-12">
        <p className="text-[10px] tracking-[0.45em] text-[#C9A84C] uppercase text-center mb-10">
        ...
        <div className="grid md:grid-cols-3 gap-5 mb-16">
          {loadingPodium
            ...
            : podium.map((p, i) => (
                <ProductCard key={p._id || p.id} product={p} medal={["🥇", "🥈", "🥉"][i]} animate delay={i * 0.1} />
              ))
          }
        </div>

        <div className="flex items-center gap-5 mb-10">
          <div className="flex-1 h-px bg-[#ECEAE6]" />
          <p className="text-[10px] tracking-[0.4em] text-[#7A7571] uppercase whitespace-nowrap">All Bestsellers</p>
          <div className="flex-1 h-px bg-[#ECEAE6]" />
        </div>
      </div>

      {/* ── Filtered grid ── */}
      <div className="max-w-screen-xl mx-auto px-5 lg:px-12 pb-16 flex gap-8">
          ...
          <AnimatePresence mode="wait">
            {loading ? (
              ...
              <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-32 text-center">
                <p className="font-cormorant text-4xl text-[#E0DBD4] mb-4">No bestsellers found</p>
              ...
              <motion.div key={`grid-${page}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-x-5 gap-y-10">
                {products.map((p, i) => (
                  <ProductCard key={p._id || p.id} product={p} rank={(page - 1) * PER_PAGE + i + 1} animate delay={(i % 4) * 0.05} />
                ))}
              </motion.div>
            )}
          </AnimatePresence>
